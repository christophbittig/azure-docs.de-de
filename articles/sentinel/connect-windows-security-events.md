...
|**Windows-Sicherheitsereignisse**     |Neuere Version **(jetzt allgemein verfügbar!)** basierend auf dem Azure Monitor-Agent (AMA).   <br><br>Unterstützt zusätzliche Features, z. B. Protokollfilterung vor der Erfassung und individuelle Datensammlungsregeln für bestimmte Gruppen von Computern.      |
|     |         |
...
# <a name="security-events-via-legacy-agent"></a>[Sicherheitsereignisse über Legacy-Agent](#tab/LAA)
...
# <a name="windows-security-events-via-azure-monitor-agent-ama"></a>[Windows-Sicherheitsereignisse über Azure Monitor Agent (AMA)](#tab/AMA)

> [!NOTE]
>
> - Der auf dem Azure Monitor-Agent (AMA) basierende Datenconnector für sicherheitsrelevante Windows-Ereignisse ist nun allgemein verfügbar.
...
> 
>   Der Azure Monitor-Agent kann parallel zu den bereits vorhandenen Agents vorhanden sein, sodass Sie den Legacy-Connector während der Bewertung oder Migration weiterhin verwenden können. Eine doppelte Datensammlung kann jedoch zu einer Verzerrung von Abfrageergebnissen führen und zusätzliche Kosten für die Datenerfassung und -aufbewahrung zur Folge haben.
> 
...
Um Ihre sicherheitsrelevanten Windows-Ereignisse in Azure Sentinel zu erfassen, befolgen Sie die Anweisungen auf der entsprechenden Registerkarte gemäß der Version des Connectors, den Sie bereitstellen möchten:

# <a name="security-events-via-legacy-agent"></a>[Sicherheitsereignisse über Legacy-Agent](#tab/LAA)

1. Klicken Sie im Azure Sentinel-Navigationsmenü auf **Data connectors** (Datenconnectors). Wählen Sie in der Liste der Connectors **Sicherheitsereignisse über Legacy-Agent** und dann im Detailbereich **Connectorseite öffnen** aus. Folgen Sie anschließend wie im Rest dieses Abschnitts beschrieben den Anweisungen auf dem Bildschirm auf der Registerkarte **Anweisungen**.
...
# <a name="windows-security-events-via-azure-monitor-agent-ama"></a>[Sicherheitsrelevante Windows-Ereignisse über Azure Monitor Agent (AMA)](#tab/AMA)

1. Klicken Sie im Azure Sentinel-Navigationsmenü auf **Data connectors** (Datenconnectors). Klicken Sie in der Liste der Connectors auf **Sicherheitsrelevante Windows-Ereignisse über AMA** und dann unten rechts auf die Schaltfläche **Connectorseite öffnen**. Folgen Sie anschließend wie im Rest dieses Abschnitts beschrieben den Anweisungen auf dem Bildschirm auf der Registerkarte **Anweisungen**.
...
1. Sie müssen RDP-Anmeldedaten (Ereignis-ID 4624) über den Datenconnector für **Sicherheitsrelevante Ereignisse über Legacy-Agent** oder für **Sicherheitsrelevante Windows-Ereignisse über AMA** erfassen. Stellen Sie sicher, dass Sie neben „Keine“ einen [Ereignissatz](#event-id-reference) ausgewählt oder eine Datensammlungsregel erstellt haben, die diese Ereignis-ID umfasst, um in Azure Sentinel zu streamen.
