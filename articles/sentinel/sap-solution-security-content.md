...
|---------|---------|--------- |
|<a name="sap---system-applications-and-products-workbook"></a>**SAP: Überwachungsprotokollbrowser**     |Zeigt Daten an wie z. B.: <br><br>Allgemeine Systemintegrität, einschließlich Benutzeranmeldungen im Zeitverlauf, vom System erfasste Ereignisse, Nachrichtenklassen und -IDs sowie ABAP-Programmausführungen <br><br>Schweregrade von Ereignissen, die in Ihrem System auftreten <br><br>Authentifizierungs- und Autorisierungsereignisse in Ihrem System |Nutzt Daten aus dem folgenden Protokoll: <br><br>[ABAPAuditLog_CL](sap-solution-log-reference.md#abap-security-audit-log) |
|**SAP: Verdächtige Berechtigungen**     |  Zeigt Daten an wie z. B.: <br><br>Sensible und kritische Zuweisungen <br><br>Aktionen und Änderungen an sensiblen, privilegierten Benutzern <br><br>Änderungen an Rollen  |Nutzt Daten aus den folgenden Protokollen: <br><br>[ABAPAuditLog_CL](sap-solution-log-reference.md#abap-security-audit-log) <br><br>[ABAPChangeDocsLog_CL](sap-solution-log-reference.md#abap-change-documents-log) |
|**SAP: Erstzugriff und Versuche zum Umgehen von SAP-Sicherheitsmechanismen**     |  Zeigt Daten an wie z. B.: <br><br>Ausführungen von sensiblen Programmen, Code und Funktionsmodulen <br><br>Konfigurationsänderungen, einschließlich Protokolldeaktivierungen <br><br>Änderungen im Debugmodus    |Nutzt Daten aus den folgenden Protokollen: <br><br>[ABAPAuditLog_CL](sap-solution-log-reference.md#abap-security-audit-log)<br><br>[ABAPTableDataLog_CL](sap-solution-log-reference.md#abap-db-table-data-log)<br><br>[Syslog](sap-solution-log-reference.md#abap-syslog) |
|**SAP: Persistenz und Datenexfiltration**     |    Zeigt Daten an wie z. B.: <br><br>ICF-Dienste (Internet Communication Framework), einschließlich Aktivierungen und Deaktivierungen sowie Daten zu neuen Diensten und Diensthandlern <br><br> Unsichere Vorgänge, einschließlich Funktionsmodule und Programme <br><br>Direkter Zugriff auf sensible Tabellen      | Nutzt Daten aus den folgenden Protokollen: <br><br>[ABAPAuditLog_CL](sap-solution-log-reference.md#abap-security-audit-log) <br><br>[ABAPTableDataLog_CL](sap-solution-log-reference.md#abap-db-table-data-log)<br><br>[ABAPSpoolLog_CL](sap-solution-log-reference.md#abap-spool-log)<br><br>[ABAPSpoolOutputLog_CL](sap-solution-log-reference.md#apab-spool-output-log)<br><br>[Syslog](sap-solution-log-reference.md#abap-syslog) |
|     |         | |

Weitere Informationen finden Sie unter [Tutorial: Visualisieren und Überwachen Ihrer Daten](monitor-your-data.md) und [Bereitstellen der kontinuierlichen Bedrohungsüberwachung in SAP](sap-deploy-solution.md).
...
|**SAP – Mittel – Mehrere Anmeldungen über dieselbe IP-Adresse**     |  Identifiziert die Anmeldung mehrerer Benutzer über dieselbe IP-Adresse innerhalb eines geplanten Zeitintervalls.   <br><br>**Untergeordneter Anwendungsfall**: [Persistenz](#built-in-sap-analytics-rules-for-persistency)    |    Melden Sie mehrere Benutzer über die gleiche IP-Adresse an. <br><br>**Datenquellen**: SAPcon – Überwachungsprotokoll | Erstzugriff        |
|**SAP – Mittel – Mehrere Anmeldungen durch Benutzer**     | Identifiziert Anmeldungen desselben Benutzers an mehreren Terminals innerhalb des geplanten Zeitintervalls.  <br><br>Bei SAP-Versionen ab 7.5 nur über die Methode „SAL überwachen“ verfügbar.      |   Melden Sie sich mit demselben Benutzer und verschiedenen IP-Adressen an.   <br><br>**Datenquellen**: SAPcon – Überwachungsprotokoll   |  PreAttack, Zugriff auf Anmeldeinformationen, Erstzugriff, Sammlung <br><br>**Untergeordneter Anwendungsfall**: [Persistenz](#built-in-sap-analytics-rules-for-persistency)      |
...
### <a name="built-in-sap-analytics-rules-for-data-exfiltration"></a>Integrierte SAP-Analyseregeln für Datenexfiltration
...
|**SAP – Niedrig – Dynamisches RFC-Ziel**     |   Identifiziert die Ausführung von RFC mithilfe dynamischer Ziele. <br><br>**Untergeordneter Anwendungsfall**: [Erstzugriff und Versuche zum Umgehen von SAP-Sicherheitsmechanismen](#built-in-sap-analytics-rules-for-attempts-to-bypass-sap-security-mechanisms)| Führen Sie einen ABAP-Bericht aus, der dynamische Ziele (cl_dynamic_destination) verwendet. Beispiel: DEMO_RFC_DYNAMIC_DEST.   <br><br>**Datenquellen**: SAPcon – Überwachungsprotokoll      |    Sammlung, Exfiltration     |
...
### <a name="built-in-sap-analytics-rules-for-persistency"></a>Integrierte SAP-Analyseregeln für Persistenz
...
### <a name="built-in-sap-analytics-rules-for-attempts-to-bypass-sap-security-mechanisms"></a>Integrierte SAP-Analyseregeln für Versuche zum Umgehen von SAP-Sicherheitsmechanismen
...
|**SAP – Hoch – Daten haben sich während der Debugaktivität geändert**     | Identifiziert Änderungen an Laufzeitdaten während einer Debugaktivität.  <br><br>**Untergeordneter Anwendungsfall**: [Persistenz](#built-in-sap-analytics-rules-for-persistency)       | 1. Aktivieren Sie das Debuggen (/h). <br>2. Wählen Sie ein zu änderndes Feld aus, und aktualisieren Sie dessen Wert.<br><br>**Datenquellen**: SAPcon – Überwachungsprotokoll        |  Ausführung/Lateral Movement       |
...
|**SAP – Hoch – Ausführung eines sensiblen Funktionsmoduls**     | Identifiziert die Ausführung eines sensiblen ABAP-Funktionsmoduls. <br><br>**Untergeordneter Anwendungsfall**: [Persistenz](#built-in-sap-analytics-rules-for-persistency)<br><br>**Hinweis**: Nur für Produktionssysteme relevant. <br><br>Verwalten Sie sensible Funktionen in der Watchlist [SAP – Sensible Funktionsmodule](#modules), und aktivieren Sie Tabellenprotokollierungsänderungen im Back-End für die Tabelle EUFUNC. (SE13)     | Führen Sie ein sensibles Funktionsmodul direkt mit SE37 aus. <br><br>**Datenquellen**: SAPcon – Tabellendatenprotokoll | Ermittlung, Command-and-Control 
...
|**SAP – Mittel – Debugaktivitäten**     |  Identifiziert alle auf das Debuggen bezogenen Aktivitäten. <br><br>**Untergeordneter Anwendungsfall**: [Persistenz](#built-in-sap-analytics-rules-for-persistency)      |Aktivieren Sie das Debuggen (/h) im System, debuggen Sie einen aktiven Prozess, fügen Sie dem Quellcode Haltepunkte hinzu usw. <br><br>**Datenquellen**: SAPcon – Überwachungsprotokoll |   Ermittlung      |
...
### <a name="built-in-sap-analytics-rules-for-suspicious-privileges-operations"></a>Integrierte SAP-Analyseregeln für Vorgänge mit verdächtigen Berechtigungen
