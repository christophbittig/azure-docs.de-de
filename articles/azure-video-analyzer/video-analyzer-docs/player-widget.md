...
```
   > [!NOTE]
   >Der `clientApiEndPoint` und das Token werden aus [Erstellen eines Tokens](./access-policies.md#creating-a-token) gesammelt.
...
## <a name="add-the-zone-drawer-component"></a>Hinzufügen der Zonenzeichnerkomponente

1. Fügen Sie dem Dokument ein AVA-Zonenzeichnerelement hinzu:
   ```html
   <ava-zone-drawer width="720px" id="zoneDrawer"></ava-zone-drawer>
   ```
1. Rufen Sie einen Link zum Video Analyzer-Zonenzeichner ab, der sich auf der Seite befindet:
   ```javascript
