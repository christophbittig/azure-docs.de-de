...
- [Azure AD-Authentifizierungsfunktionen für Azure SQL MI](https://azure.microsoft.com/updates/azure-ad-authentication-features-for-azure-sql-db-azure-synapse-analytics-and-azure-sql-managed-instance/) - Diese Funktionen helfen bei der Automatisierung der Benutzererstellung mit Azure AD-Anwendungen und ermöglichen die Erstellung einzelner Azure AD-Gastbenutzer in der Azure SQL Managed Instance (Public Preview).
...
Um zu überprüfen, ob der Dienstprinzipal vorhanden ist, navigieren Sie im Azure-Portal zur Seite _Unternehmensanwendungen_, wählen in der Dropdownliste _Anwendungstyp_ die Option _Verwaltete Identitäten_ aus, klicken auf _Übernehmen_ und geben den Namen der verwalteten Instanz in das Suchfeld ein. Wenn der Instanzname in der Ergebnisliste enthalten ist, sind keine weiteren Aktionen erforderlich, da der Dienstprinzipal bereits vorhanden ist.

Wenn Sie bereits die Anweisungen in der Fehlermeldung befolgt und in der Fehlermeldung auf den Link geklickt haben, wurde der Dienstprinzipal der verwalteten Instanz neu erstellt. Weisen Sie in diesem Fall dem neu erstellten Dienstprinzipal Azure AD-Leseberechtigungen zu, damit die Azure AD-Authentifizierung ordnungsgemäß funktioniert. Dies kann über Azure PowerShell anhand [dieser Anweisungen](./authentication-aad-configure.md?tabs=azure-powershell#powershell) erfolgen.
...
Nachdem [logischer Server](./logical-servers.md) gelöscht wurde, gibt es eine Schwellenfrist von 7 Tagen, bevor der Name aus den Aufzeichnungen freigegeben wird. In diesem Zeitraum kann keine SQL Managed Instance mit demselben Namen erstellt werden. Als Umgehungslösung müssen Sie einen anderen Namen für die SQL Managed Instance verwenden oder ein Supportticket für die Freigabe eines logischen Servernamens erstellen.
...
### <a name="tempdb-structure-and-content-is-re-created"></a>Die Tempdb-Struktur und der Inhalt werden neu erstellt
