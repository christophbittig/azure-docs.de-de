---
title: Host-Schlüssel für Secure File Transfer Protocol (SFTP) Unterstützung in Azure Blob Storage (Vorschau) | Microsoft Docs
description: Hier finden Sie eine Liste gültiger Hostschlüssel, wenn Sie einen SFTP-Client für die Verbindung mit Azure Blob Storage verwenden.
author: normesta
ms.subservice: blobs
ms.service: storage
ms.topic: reference
...
ms.locfileid: "132557299"
---
# <a name="host-keys-for-secure-file-transfer-protocol-sftp-support-in-azure-blob-storage-preview"></a>Host-Schlüssel für Secure File Transfer Protocol (SFTP) Unterstützung in Azure Blob Storage (Vorschau)

Dieser Artikel enthält eine Liste gültiger Hostschlüssel, die für die Verbindung zu Azure Blob Storage von SFTP-Clients aus verwendet werden.

Der Blob-Speicher unterstützt jetzt das Secure File Transfer Protocol (SFTP). Diese Unterstützung bietet die Möglichkeit, über einen SFTP-Endpunkt eine sichere Verbindung zu Blob-Storage-Konten herzustellen, so dass Sie SFTP für den Dateizugriff, die Dateiübertragung und die Dateiverwaltung nutzen können. Weitere Informationen finden Sie unter [Secure File Transfer (SFTP) Protokollunterstützung in Azure Blob Storage](secure-file-transfer-protocol-support.md).

Wenn Sie mit einem SFTP-Client eine Verbindung zu Blob Storage herstellen, werden Sie möglicherweise aufgefordert, einem Hostschlüssel zu vertrauen. Während der öffentlichen Vorschau können Sie den Host-Schlüssel überprüfen, indem Sie ihn in der in diesem Artikel vorgestellten Liste suchen. 

> [!IMPORTANT]
> Die SFTP-Unterstützung befindet sich derzeit in der VORSCHAU und ist in [diesen Regionen](secure-file-transfer-protocol-support.md#regional-availability) verfügbar.
> 
...
## <a name="valid-host-keys"></a>Gültige Hostschlüssel

> [!div class="mx-tdBreakAll"]
> | Region | Host-Schlüsseltyp | SHA 256 Fingerabdruck <sup>1</sup> | Öffentlicher Schlüssel |
> |---|---|---|---|
...
<sup>1</sup>    Der Fingerabdruck SHA 256 wird von Open SSH und WinSCP verwendet.
