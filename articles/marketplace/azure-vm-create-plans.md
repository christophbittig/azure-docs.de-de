...
- **Link zur Betriebssystem-VHD**: Das Image, das in der Azure Compute Gallery gespeichert ist (ehemals Shared Image Gallery). Erfahren Sie, wie Sie Ihr Image in [Azure Compute Gallery](azure-vm-create-using-approved-base.md#capture-image) erfassen.
...
> [!NOTE]
> Wenn Sie Ihre Images mit einer SAS bereitstellen und über Datenträger verfügen, müssen Sie diese ebenfalls als SAS-URI bereitstellen. Wenn Sie freigegebene Images verwenden, werden diese als Teil Ihres Images in der Azure Compute Gallery erfasst. Nachdem Ihr Angebot im Azure-Marktplatz veröffentlicht wurde, können Sie das Image aus Ihrem Azure-Speicher oder aus der Azure Compute Gallery löschen.
